...
2. **An example unit** about *el ciclo del agua*, in the form of an **ELPX extracted at the repository root** (`index.html`, `content.xml`, `content.dtd`, `html/`, `idevices/`, `libs/`, `content/`, `search_index.js`). Serving this root with any static HTTP server is enough to preview the style in action.
   - The canonical packaged form is `el-ciclo-del-agua.elpx`, generated by CI on every tag push and published as a release asset. Opening it live in eXeLearning:
...
The example unit is generated from a Python script kept in `scripts/build_water_cycle.py`. The output of the pipeline is the **unzipped workspace at the repo root** — the canonical `.elpx` release asset is built by CI on every tag push.
...
- Two action buttons on intro and credits: **Abrir en eXeLearning** (`static.exelearning.dev/?url=https://github.com/ateeducacion/exelearning-style-scumm/releases/latest/download/el-ciclo-del-agua.elpx`) and **Descargar estilo** (GitHub latest release `exelearning-style-scumm-<version>.zip`).
...
- **Re-export the sample ELPX with 11 nodes** now that `pp_theme=scumm` and `scripts/build_water_cycle.py` covers the full 11-page structure. The current committed root may still reflect an earlier 5-page export.
...
- **Implement scanlines, tweaks panel, fade transition, and intro loader** — the four features listed as "planned" in §5. They are described architecturally; the feature agent has the implementation spec.
- **Responsive tweaks panel on narrow viewports** — the planned `#scummTweaks` overlay needs a mobile-safe layout that does not collide with the bottom panel.
- **Mobile bottom-panel collapse** — on viewports < 768 px the panel stacks vertically and can exceed 60 vh; a collapse-to-icon-row mode is pending.
- **Release pipeline.** `.github/workflows/release.yml` exists; verify it produces both the theme ZIP and `el-ciclo-del-agua.elpx` correctly after the 11-node rebuild.
